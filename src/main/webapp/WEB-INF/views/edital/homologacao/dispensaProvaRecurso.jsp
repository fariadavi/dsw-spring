...
<div id="contents" data-ng-controller="dispensaController as ctrl">
   <div class="mdl-grid">
        <div class="mdl-cell mdl-cell--12-col page-header">
			<h3><spring:message code="edital.dispensa.recurso.title"/></h3>
		</div>
	</div>
				...
				<table data-ng-table="tableParams" class="mdl-data-table mdl-js-data-table mdl-shadow--2dp wide paginated" style="font-size: 12px"> 
					<tr data-ng-repeat="item in $data"> 
						...
						<td class="mdl-data-table__cell--non-numeric" header-class="'text-left'" data-title="'<spring:message code='edital.dispensa.prova.inicial.table'/>'">
        					<input class="" type="checkbox" data-ng-model="item.dispensaProvaRecurso" name="chkSelecao" data-ng-change="ctrl.dispensarProvaRecurso(item.id,item.dispensaProvaRecurso,item.justificativaDispensaProvaRecurso)" />
						</td>
						<td class="mdl-data-table__cell--non-numeric" header-class="'text-left'" data-title="'<spring:message code='edital.dispensa.prova.recurso.table.justificativa'/>'">
							<textarea rows="2" cols="50" style="resize:none;" ng-model="item.justificativaDispensaProvaRecurso"></textarea>
						</td>					
