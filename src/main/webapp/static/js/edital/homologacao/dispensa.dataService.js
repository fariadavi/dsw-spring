App.factory("dispensaDataService", ["$http", function ($http) {
	return {
		listaDispensaProva: function(params) {
			return $http.get(contextPath + "/edital/homologacao/dispensa/" + params.comissao + "?page=" + params.page + "&size=" + params.size + "&nome=" + (params.nome || "") + "&status=" + (params.statusDispensa || ""));
		},
